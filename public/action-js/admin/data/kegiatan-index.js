...
$(document).ready(function(){
  ...
  $('#menu_kegiatan').addClass('active');
  $('#menu_data_parent').addClass('pcoded-trigger');

  $('#all-kegiatan').DataTable();
  $('.user-tambah').hide();

  loaddata('data_kegiatan');
  loaddata('data_komponen');
  loaddata('data_pelaksana');

  $('#save-kegiatan').on('click', function(){
      let param = 'data_kegiatan';
      let id_program = $('#pilih_program').val();
      let nama_kegiatan = $('#nama_kegiatan').val();
      let id_pelaksana = $('#pilih_pelaksana').val();
      let value = $('#value_kegiatan').val();

      var formData = new FormData();
      formData.append('param', param);
      formData.append('id_program', id_program);
      formData.append('nama_kegiatan', nama_kegiatan);
      formData.append('id_pelaksana', id_pelaksana);
      ...
      success: function(result){
          ...
          if(param == 'data_pelaksana'){
            ...
            for (var i = 0; i < data.length; i++) {
              opt += '<option value="'+data[i].id+'">'+data[i].nama_pelaksana+'</option>';
            }
            $('#pilih_pelaksana').html(opt);
            ...
            let opt = '<option> - Pilih - </option>'
            for (var i = 0; i < data.length; i++) {
              opt += '<option value="'+data[i].id+'">'+data[i].nama_program+'</option>';
            }
            $('#pilih_program').html(opt);
          }else{
